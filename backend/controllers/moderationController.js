...
const createNotification = async ({
  ...
  try {
    ...
    console.error(
      "Create moderation notification failed:",
      error.message || error,
    );
  }
};
...
const addReport = async (req, res) => {
  try {
    ...
    if (!review_id) {
      return res
        .status(400)
        .json({ success: false, message: "review_id is required" });
    }
    ...
    if (!review.event?.organizer_id) {
      return res
        ...
        .json({
          success: false,
          message:
            "This review cannot be reported because organizer data is missing",
        });
    }
    ...
    if (!reportedUser) {
      return res
        ...
        .json({
          success: false,
          message:
            "This review cannot be reported because the reported user account is missing",
        });
    }
    ...
    if (!organizer) {
      return res
        ...
        .json({
          success: false,
          message:
            "This review cannot be reported because the organizer account is missing",
        });
    }
...
const addEventReport = async (req, res) => {
  try {
    ...
    if (!event.organizer_id) {
      return res
        ...
        .json({
          success: false,
          message:
            "This event cannot be reported because organizer data is missing",
        });
    }
    ...
    if (event.organizer_id === reporterId) {
      return res
        .status(400)
        .json({ success: false, message: "You cannot report your own event" });
    }
    ...
    if (!organizer) {
      return res
        ...
        .json({
          success: false,
          message:
            "This event cannot be reported because organizer account is missing",
        });
    }
    ...
    await runNonBlocking("Send reporter event-report email", async () => {
      await sendModerationEmail({
        ...
        details: [`Event: ${event.title}`, `Reason: ${normalizedReason}`],
        ctaLabel: "View Event",
...
const getOrganizerReportById = async (req, res) => {
  try {
    ...
    if (
      !report ||
      report.scope !== "organizer_user" ||
      report.organizer_id !== req.user.id
    ) {
      return res
        .status(404)
...
const decideOrganizerReport = async (req, res) => {
  try {
    ...
    if (!action) {
      return res
        .status(400)
        .json({ success: false, message: "action must be ban or reject" });
    }
    ...
    if (report.status !== "pending") {
      return res
        .status(400)
        .json({ success: false, message: "Report is already resolved" });
    }
    ...
    if (action === "ban" && !report.subject_user_id) {
      return res
        ...
        .json({
          success: false,
          message:
            "This report cannot be resolved with a ban because the reported user is missing",
        });
    }
    ...
    if (action === "ban") {
      ...
      await sendModerationEmail({
        ...
        ctaLabel: "View Event",
        ctaUrl: report.event?.id
          ? `${FRONTEND_URL}/event/${report.event.id}`
    ...
    await sendModerationEmail({
      ...
      details: [note ? `Decision note: ${note}` : null].filter(Boolean),
  ...
  try {
    if (!isSuperAdmin(req.user)) {
      return res
        ...
        .json({
          success: false,
          message: "Only super admin can decide reports",
        });
    }
    ...
    if (action === "ban" && !report.subject_user_id) {
      return res
        ...
        .json({
          success: false,
          message:
            "This report cannot be resolved with a ban because the organizer account is missing",
        });
    }
    ...
    if (action === "ban") {
      ...
      const organizerDisplayName =
        report.subject_user?.organizer_profile?.organization_name ||
      ...
      await createNotification({
        ...
        message: `Your organization was banned by super admin.`,
        metadata: {
          action: "open_ban_appeal",
          ban_id: activeBan.id,
          scope: "platform_organizer",
      ...
      await sendModerationEmail({
        ...
        intro:
          "Super admin reviewed a report and applied a platform ban to your organizer account.",
        details: [
          `Reason: ${activeBan.reason}`,
          note ? `Decision note: ${note}` : null,
...
const submitAppeal = async (req, res) => {
  try {
    ...
    if (!ban_id) {
      return res
        .status(400)
        .json({ success: false, message: "ban_id is required" });
    }

    if (!normalizedMessage) {
      return res
        .status(400)
        .json({ success: false, message: "Appeal message is required" });
    }
    ...
    if (!ban || ban.status !== "active") {
      return res
        .status(404)
        .json({ success: false, message: "Active ban not found" });
    }

    if (ban.subject_user_id !== req.user.id) {
      return res
        .status(403)
        .json({ success: false, message: "You can only appeal your own ban" });
    }
    ...
    await sendModerationEmail({
      ...
      details: [`Appeal message: ${normalizedMessage}`, `Scope: ${ban.scope}`],
      ctaLabel: "Open Notifications",
      ctaUrl: `${FRONTEND_URL}/discover`,
    });
    ...
    await sendModerationEmail({
      ...
      details: [`Scope: ${ban.scope}`, `Appeal message: ${normalizedMessage}`],
      ctaLabel:
        ban.scope === "organizer_user"
          ? "Open Organizer Dashboard"
          : "Open Admin Dashboard",
      ctaUrl:
        ban.scope === "organizer_user"
          ? `${FRONTEND_URL}/organizer/dashboard`
...
const getOrganizerAppealById = async (req, res) => {
  try {
    ...
    if (
      !appeal ||
      appeal.ban.scope !== "organizer_user" ||
      appeal.ban.organizer_id !== req.user.id
    ) {
      return res
        .status(404)
...
const decideOrganizerAppeal = async (req, res) => {
  try {
    ...
    if (appeal.status !== "pending") {
      return res
        .status(400)
        .json({ success: false, message: "Appeal is already resolved" });
    }
    ...
    if (appeal.ban.status !== "active") {
      return res
        ...
        .json({
          success: false,
          message:
            "Appeal cannot be decided because the ban is no longer active",
        });
    }
    ...
    if (action === "approve") {
      ...
      await sendModerationEmail({
        ...
        intro:
          "Your organizer ban appeal was approved and your restriction has been removed.",
        details: [note ? `Decision note: ${note}` : null].filter(Boolean),
        ctaLabel: "Browse Events",
        ctaUrl: `${FRONTEND_URL}/discover`,
      });
  ...
  try {
    if (!isSuperAdmin(req.user)) {
      return res
        ...
        .json({
          success: false,
          message: "Only super admin can access appeals",
        });
    }
  ...
  try {
    if (!isSuperAdmin(req.user)) {
      return res
        ...
        .json({
          success: false,
          message: "Only super admin can decide appeals",
        });
    }
    ...
    if (action === "approve") {
      ...
      await createNotification({
        ...
        title: "Appeal approved",
        message:
          "Your appeal was approved and the organization ban has been lifted.",
        metadata: {
          action: "open_appeal",
          appeal_id: appeal.id,
          decision: "approved",
    ...
    await createNotification({
      ...
      title: "Appeal rejected",
      message:
        "Your appeal was rejected and the organization ban remains active.",
      metadata: {
        action: "open_appeal",
        appeal_id: appeal.id,
        decision: "rejected",
...
const getBanById = async (req, res) => {
  try {
    ...
    const canAccess =
      ban.subject_user_id === req.user.id ||
    ...
    if (!canAccess) {
      return res
        .status(403)
        .json({ success: false, message: "Not authorized to access this ban" });
    }
