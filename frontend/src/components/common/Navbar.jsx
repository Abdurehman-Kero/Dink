        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link to="/admin/dashboard" className="flex items-center gap-3 group">
            ...
            <div>
              ...
              <span className="ml-2 text-xs bg-red-500 px-2 py-0.5 rounded-full font-semibold">ADMIN</span>
            </div>
          </Link>
      ...
      <nav className="sticky top-0 z-50 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 shadow-sm">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <Link to="/organizer/dashboard" className="flex items-center gap-3 group">
            ...
            <div>
              <span className="text-xl font-bold text-gray-900 dark:text-white">DEMS</span>
              <span className="ml-2 text-xs text-green-600 dark:text-green-400 font-medium">Organizer</span>
            </div>
          </Link>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <Link to="/" className="flex items-center gap-2 group shrink-0">
            ...
            <span className="font-bold text-xl text-gray-900 tracking-tight">DEMS</span>
          </Link>

          {/* Desktop Navigation Links */}
          <div className="hidden md:flex items-center gap-2 ml-8">
            <Link to="/discover" className={`relative px-4 py-2 text-sm font-medium rounded-lg transition ${location.pathname === '/discover' ? 'text-green-600 bg-green-50' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'}`}>
              Discover Events
            </Link>
            
            {isAuthenticated && (
              <>
                <Link to="/saved-tickets" className={`relative px-4 py-2 text-sm font-medium rounded-lg transition flex items-center gap-2 ${location.pathname === '/saved-tickets' ? 'text-green-600 bg-green-50' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'}`}>
                  <Heart className="size-4" /> Saved
                </Link>
                <Link to="/my-tickets" className={`relative px-4 py-2 text-sm font-medium rounded-lg transition flex items-center gap-2 ${location.pathname === '/my-tickets' ? 'text-green-600 bg-green-50' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'}`}>
                ...
                {showProfileMenu && (
                  <>
                    ...
                    <div className="absolute right-0 mt-2 w-56 rounded-xl shadow-xl border py-2 z-50 bg-white border-gray-100">
                      ...
                      <Link to="/saved-tickets" onClick={() => setShowProfileMenu(false)} className="flex items-center gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                        <Heart className="size-4" /> Saved Tickets
                      </Link>
                      <Link to="/my-tickets" onClick={() => setShowProfileMenu(false)} className="flex items-center gap-3 px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                        <Ticket className="size-4" /> My Tickets
                      </Link>
        ...
        {showMobileMenu && (
          <div className="md:hidden py-4 border-t border-gray-100 animate-slideDown">
            <div className="space-y-2">
              <Link to="/discover" className="flex items-center gap-3 px-4 py-3 text-gray-700 hover:bg-gray-50 rounded-lg" onClick={() => setShowMobileMenu(false)}>
                <Calendar className="size-4" /> Discover Events
              </Link>
              {isAuthenticated && (
                <>
                  <Link to="/saved-tickets" className="flex items-center gap-3 px-4 py-3 text-gray-700 hover:bg-gray-50 rounded-lg" onClick={() => setShowMobileMenu(false)}>
                    <Heart className="size-4" /> Saved Tickets
                  </Link>
                  <Link to="/my-tickets" className="flex items-center gap-3 px-4 py-3 text-gray-700 hover:bg-gray-50 rounded-lg" onClick={() => setShowMobileMenu(false)}>
                    <Ticket className="size-4" /> My Tickets
                  </Link>
                  <Link to="/profile" className="flex items-center gap-3 px-4 py-3 text-gray-700 hover:bg-gray-50 rounded-lg" onClick={() => setShowMobileMenu(false)}>
