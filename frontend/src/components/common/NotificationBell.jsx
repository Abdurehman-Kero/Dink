...
import { Bell, BellRing, CheckCircle, Calendar, Ticket, Users, ShieldAlert, FileWarning, Scale } from 'lucide-react';
...
export function NotificationBell() {
  ...
  const getNotificationIcon = (type) => {
    switch(type) {
      ...
      case 'ban': return <ShieldAlert className="size-4 text-red-500" />;
      case 'report_submitted': return <FileWarning className="size-4 text-orange-500" />;
      case 'report_decision': return <FileWarning className="size-4 text-blue-500" />;
      case 'appeal_submitted': return <Scale className="size-4 text-amber-500" />;
      case 'appeal_decision': return <Scale className="size-4 text-green-500" />;
                ...
                {notifications.map(notif => (
                  ...
                  >
                    <div className="flex gap-3">
                      ...
                      {!notif.read_at && (
                        <button onClick={(e) => { e.stopPropagation(); markAsRead(notif.id); }} className="text-xs text-green-600 hover:text-green-700">
      ...
      {showAppealModal && (
        <div className="fixed inset-0 z-[60] bg-black/50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-lg p-6">
            ...
            <p className="text-sm text-gray-500 mb-4">You can submit an appeal for this moderation decision.</p>
              ...
              <>
                <div className="rounded-xl border border-red-200 bg-red-50 p-3 mb-4">
                  <p className="text-sm text-red-700 font-medium mb-1">
                    {selectedBanNotification?.message || 'Your booking or organizer access has been restricted.'}
                  </p>
                  <p className="text-xs text-red-600">
                    Organizer: {banDetails?.organizer?.organizer_profile?.organization_name || banDetails?.organizer?.full_name || selectedBanNotification?.metadata?.organizer_name || 'N/A'}
                  </p>
                  <p className="text-xs text-red-600 mt-1">
                    Reason: {banDetails?.reason || selectedBanNotification?.metadata?.reason || 'No reason provided'}
                  </p>
                </div>

                <label className="block text-sm font-medium text-gray-700 mb-1">Appeal Message</label>
                <textarea
                  rows={5}
                  value={appealMessage}
