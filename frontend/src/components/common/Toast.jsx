...
export function ToastContainer() {
  ...
  useEffect(() => {
    const handleShow = (e) => {
      setVisibleToasts(prev => [...prev, { id: e.detail.id, title: e.detail.title }]);
    };
    const handleHide = (e) => {
  ...
  return (
    <div className="fixed bottom-24 right-6 z-50 space-y-2">
      {visibleToasts.map(toast => (
        <div key={toast.id} className="bg-white rounded-xl shadow-lg p-4 border-l-4 border-green-500 animate-slideUp min-w-[280px]">
          <div className="flex items-start gap-3">
            <CheckCircle className="size-5 text-green-500 flex-shrink-0 mt-0.5" />
            <div className="flex-1">
              <p className="text-sm font-semibold text-gray-900">Event Saved!</p>
