...
const ethiopianCities = {
  ...
  'Mekelle': { lat: 13.5000, lng: 39.4667 },
  'Jimma': { lat: 7.6667, lng: 36.8333 },
  'Harar': { lat: 9.3167, lng: 42.1167 }
};

export function EventMap({ events, selectedEvent, onEventSelect, height = "400px" }) {
  ...
  const sortedEvents = userLocation && showUserLocation
    ? [...events].sort((a, b) => {
        const distA = calculateDistance(userLocation.lat, userLocation.lng, a.latitude || ethiopianCities[a.city]?.lat || 9.0320, a.longitude || ethiopianCities[a.city]?.lng || 38.7469);
        const distB = calculateDistance(userLocation.lat, userLocation.lng, b.latitude || ethiopianCities[b.city]?.lat || 9.0320, b.longitude || ethiopianCities[b.city]?.lng || 38.7469);
        return distA - distB;
      })
    : events;
      ...
      <div style={{ height, position: 'relative' }}>
        ...
        >
          ...
          {sortedEvents.map((event, idx) => {
            const lat = event.latitude || ethiopianCities[event.city]?.lat || 9.0320;
            const lng = event.longitude || ethiopianCities[event.city]?.lng || 38.7469;
            const distance = userLocation && showUserLocation 
              ...
              >
                <Popup>
                  <div className="min-w-[200px] p-2">
                    ...
                    <p className="text-xs text-gray-600 mt-1">{event.venue_name || event.city}</p>
                    <p className="text-xs text-green-600 mt-1">
                      {new Date(event.start_datetime).toLocaleDateString()}
                    </p>
                    {distance && (
                      <p className="text-xs text-blue-600 mt-1">Distance: {distance} km away</p>
                    )}
                    <button
              ...
              >
                <div className="flex justify-between items-start">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-1">{event.venue_name || event.city}</p>
                  </div>
                  {distance && (
                    <span className="text-xs text-green-600 whitespace-nowrap ml-2">{distance} km</span>
                  )}
                </div>
              </div>
            );
