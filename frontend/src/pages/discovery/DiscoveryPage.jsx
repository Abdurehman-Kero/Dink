...
import { 
  ...
  SlidersHorizontal, Coffee, Music, Mic, Palette, Briefcase, 
  GraduationCap, Zap, Clock, DollarSign, Tag, Sparkles,
  Award, Eye, Share2, Bookmark, ExternalLink
} from 'lucide-react';
...
export function DiscoveryPage() {
  ...
  const categories = [
    { id: 'all', name: 'All Events', icon: Sparkles, color: 'from-gray-500 to-gray-600' },
    ...
    { id: 'Technology', name: 'Technology', icon: Coffee, color: 'from-blue-500 to-cyan-500' },
    { id: 'Business', name: 'Business', icon: Briefcase, color: 'from-emerald-500 to-teal-500' },
    { id: 'Educational', name: 'Educational', icon: GraduationCap, color: 'from-indigo-500 to-purple-500' },
    { id: 'Sports', name: 'Sports', icon: Zap, color: 'from-red-500 to-orange-500' }
  ];
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      if (response.success) {
        let publishedEvents = (response.events || []).filter(e => e.status === 'published');
        ...
        publishedEvents = publishedEvents.map(event => ({
          ...
          min_price: event.ticket_types?.length > 0 
            ...
            : 0,
          total_tickets: event.ticket_types?.reduce((sum, t) => sum + (t.capacity - t.remaining_quantity), 0) || 0
        }));
  ...
  useEffect(() => {
    ...
    setCurrentPage(1);
  }, [searchQuery, selectedCategory, selectedCity, priceRange, events, sortBy, showSavedOnly, savedEvents]);
      ...
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              Filters
              {(selectedCity || priceRange.min || priceRange.max || showSavedOnly) && (
        ...
        {showFilters && (
          <div className="bg-white rounded-2xl p-6 mb-8 shadow-lg border border-gray-100 animate-fadeIn">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-semibold text-gray-900 text-lg">Filter Events</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
              ...
              <div className="flex items-end">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-gray-700">Show only saved events</span>
                </label>
              </div>
            </div>
        ...
        {currentEvents.length > 0 ? (
          ...
          <div className="text-center py-16 bg-white rounded-2xl shadow-sm">
            ...
            <button onClick={clearFilters} className="px-6 py-2 bg-green-600 text-white rounded-xl hover:bg-green-700 transition">
              Clear all filters
            </button>
          </div>
        )}
...
function EventCard({ event, formatDate, viewMode }) {
  ...
  const isEventToday = new Date(event.start_datetime).toDateString() === new Date().toDateString();
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2 mb-2 flex-wrap">
                  ...
                  {isEventToday && (
                    <span className="px-2 py-1 bg-green-100 text-green-700 text-xs rounded-lg animate-pulse">Today</span>
                  )}
                  {event.is_trending && (
                ...
                <div className="flex flex-wrap gap-4 text-sm text-gray-500">
                  <span className="flex items-center gap-1"><Calendar className="size-4 text-green-600" />{formatDate(event.start_datetime)}</span>
                  <span className="flex items-center gap-1"><MapPin className="size-4 text-red-500" />{event.city}</span>
                </div>
              </div>
              <div className="text-right">
                <div className="text-2xl font-bold text-green-600">ETB {event.min_price || 250}+</div>
                <button 
                  onClick={(e) => {
                    e.preventDefault();
                    const savedEvents = JSON.parse(localStorage.getItem('savedEvents') || '[]');
        ...
        <div className="relative h-52 overflow-hidden">
          ...
          {isEventEnded && (
            <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
              <span className="px-3 py-1 bg-gray-800 text-white text-sm font-semibold rounded-full">Event Ended</span>
            </div>
          )}
        </div>
