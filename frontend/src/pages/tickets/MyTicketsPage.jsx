...
export function MyTicketsPage() {
  ...
  useEffect(() => {
    const purchased = localStorage.getItem('purchasedTickets');
    if (purchased) {
      setTickets(JSON.parse(purchased));
    }
    setLoading(false);
  }, []);
  ...
  const downloadTicket = (ticket) => {
    ...
    printWindow.document.write(`
      ...
      <div><p><strong>Date:</strong> ${new Date(ticket.event.date).toLocaleDateString()}</p><p><strong>Location:</strong> ${ticket.event.location}</p><p><strong>Ticket Type:</strong> ${ticket.ticket_type.tier_name}</p><p><strong>Quantity:</strong> ${ticket.quantity}</p><p><strong>Ticket Code:</strong> ${ticket.ticket_code}</p></div>
